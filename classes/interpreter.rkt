...
(define (value-of exp Δ)
  (match exp
    ...
    [(ast:let (ast:var x) e1 e2) (begin (display (value-of e1 Δ))(value-of e2 (extend-env x (newref (value-of e1 Δ)) Δ)))]
    ...
    [(ast:new (ast:var n) args) empty-env ]
...
(define (value-of-class decl Δ)
  (match decl
    [(ast:decl name super fields methods)
      (begin
        (extend-env (class-name name) decl (value-of-fields fields Δ))
      )
    ]
    [(list c1 cr)
      (begin 
...
(define (value-of-fields fields Δ)
  ...
  (match fields
    ...
    [(list f1 fr)
    (begin 
      (value-of-fields f1 Δ)
      (value-of-fields fr Δ)
    )]
 )
)
